...
def sync_user_to_faculty_student(sender, instance, created, **kwargs):
    ...
    try:
        with transaction.atomic():
            ...
            if instance.role == "faculty":
                ...
                if faculty:
                    ...
                    faculty.save()
                    logger.info(
                        f"Updated faculty {faculty.faculty_id} from user {instance.username}"
                    )
                elif created:
                    # Create faculty record for new faculty user (optional - can be manual)
                    logger.info(
                        f"Faculty user {instance.username} created, but no Faculty record exists yet"
                    )
            ...
            # Check if this user is a student
                ...
                if student:
                    ...
                    student.save()
                    logger.info(
                        f"Updated student {student.student_id} from user {instance.username}"
                    )
                elif created:
                    # Create student record for new student user (optional - can be manual)
                    logger.info(
                        f"Student user {instance.username} created, but no Student record exists yet"
                    )
        ...
        logger.error(
            f"Error syncing user {instance.username} to Faculty/Student: {str(e)}"
        )
    ...
    try:
        with transaction.atomic():
            ...
            if user:
                ...
                user.save()
                logger.info(
                    f"Updated user {user.username} from faculty {instance.faculty_id}"
                )
            else:
                ...
                user.save()
                logger.info(
                    f"Created user {user.username} for faculty {instance.faculty_id}"
                )
...
def delete_faculty_user(sender, instance, **kwargs):
    ...
    try:
        ...
        if user:
            user.delete()
            logger.info(
                f"Deleted user {user.username} for faculty {instance.faculty_id}"
            )
            _invalidate_user_caches(instance.email)
    except Exception as e:
    ...
    try:
        with transaction.atomic():
            ...
            if user:
                ...
                user.save()
                logger.info(
                    f"Updated user {user.username} from student {instance.student_id}"
                )
            else:
                ...
                user.save()
                logger.info(
                    f"Created user {user.username} for student {instance.student_id}"
                )
...
def delete_student_user(sender, instance, **kwargs):
    ...
    try:
        ...
        if user:
            user.delete()
            logger.info(
                f"Deleted user {user.username} for student {instance.student_id}"
            )
            _invalidate_user_caches(instance.email)
    except Exception as e:
