...
class GenerationJobViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == 'admin':
            ...
            return GenerationJob.objects.filter(department__department_id=user.department)
    ...
    def generate_timetable(self, request):
        ...
        try:
            department = Department.objects.get(department_id=serializer.validated_data['department_id'])
    ...
    def get_result(self, request, pk=None):
        ...
        try:
            ...
            timetables = Timetable.objects.filter(generation_job=job).prefetch_related('slots')
    ...
    def _queue_generation_job(self, job):
        ...
        try:
            ...
            try:
                requests.post(
                    f'{fastapi_url}/api/generate/{job.job_id}',
                    json=job_data,
                    timeout=5
                )
                logger.info(f"Triggered FastAPI generation for job {job.job_id}")
            except requests.exceptions.RequestException as e:
                logger.warning(f"Could not trigger FastAPI service: {e}. Job queued in Redis.")
