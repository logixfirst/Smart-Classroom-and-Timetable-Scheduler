...
class Command(BaseCommand):
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                try:
                    ...
                    self.stdout.write(self.style.WARNING(f'Skipping subject {row["subject_id"]}: {e}'))
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                try:
                    ...
                    department = Department.objects.filter(department_name__icontains=dept_name[:3]).first()
                    if not department:
                    ...
                    faculty, created = Faculty.objects.get_or_create(
                        ...
                        defaults={
                            ...
                            'max_workload_per_week': int(row.get('max_workload_per_week', 18)),
                            'email': email,
                            'phone': phone
                        }
                    )
                    ...
                    if created:
                        User.objects.create(
                            ...
                            last_name=' '.join(row['faculty_name'].split()[1:]) if len(row['faculty_name'].split()) > 1 else ''
                        )
                    count += 1
                except Exception as e:
                    self.stdout.write(self.style.WARNING(f'Skipping faculty {row.get("faculty_id", "unknown")}: {e}'))
        self.stdout.write(self.style.SUCCESS(f'✅ Imported {count} faculty members with user accounts'))
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                try:
                    ...
                    dept_name = row.get('department', 'Computer Science')
                    department = dept_map.get(dept_name) or next(iter(dept_map.values()))
                    
                    course_name = row.get('course', 'B.Tech')
                    course = course_map.get(course_name) or next(iter(course_map.values()))
                    ...
                    if len(students_batch) >= batch_size:
                        ...
                        self.stdout.write(f'  Imported {count} students...', ending='\r')
                        students_batch = []
                        users_batch = []
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                try:
                    ...
                    self.stdout.write(self.style.WARNING(f'Skipping batch {row["batch_id"]}: {e}'))
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                try:
                    ...
                    self.stdout.write(self.style.WARNING(f'Skipping classroom {row["room_id"]}: {e}'))
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                try:
                    ...
                    self.stdout.write(self.style.WARNING(f'Skipping lab {row["lab_code"]}: {e}'))
    ...
    def create_default_users(self):
        ...
        for user_data in default_users:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f'✅ Created user: {user_data["username"]} (password: {user_data["password"]})'))
            else:
                self.stdout.write(self.style.WARNING(f'⚠️  User already exists: {user_data["username"]}'))
